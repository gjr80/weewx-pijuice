...
class PiJuiceApi(object):
    ...
    def status(self):
        ...
        Obtain the PiJuice status via the API. The API response is a dict keyed
        as follows:
    ...
    def button_events(self):
        """Obtain the events generated by PiJuice buttons presses.

        Obtain the events generated by PiJuice buttons presses via the API. The
        API response is a dict keyed as follows:
    ...
    @property
    def led1_state(self):
        """Obtain the LED1 state.

        Obtain the LED1 state via the API. The API response is a dict keyed as
        follows:
    ...
    @property
    def led2_state(self):
        """Obtain the LED2 state.

        Obtain the LED2 state via the API. The API response is a dict keyed as
        follows:
...
class DirectPiJuice(object):
    ...
    def display_status(self):
        ...
        if 'error' not in resp:
            ...
            for key, value in resp.items():
                # display the raw status string or a formatted version
                if self.args.raw:
    ...
    def display_button_events(self):
        """Display the PiJuice button events."""

        # get the PiJuice button events
        resp = self.pj.button_events
        print()
        print("PiJuice button events:")
        # If the API encountered an error when obtaining the PiJuice button
        # events there will be an 'error' field in the API response. If there
        # was no error display the PiJuice button events. Otherwise display the
        # error in formatted text or as the raw error string.
        if 'data' in resp:
            # iterate over the data field fields in the response
            for key, value in resp['data'].items():
                # display the raw button event data or a formatted version
                if self.args.raw:
                    # --raw was set so display the raw button event data
                    print("%8s: %s" % (key, value))
                else:
                    # --raw was not set so display formatted button event data
                    print("%8s: %s" % (key, button_events.get(value, value)))
        else:
            # we have an error, display it
            print(self.display_error(resp['error']))
        return
...
"""
    ...
    parser.add_argument('--button-events', dest='buttons', action='store_true',
